...
function* run() {
  ...
  const data = fs.readFileSync('./temp/dates/dates.csv', 'utf-8');
  ...
  for (let i = 0; i < dates.length; i++) {
    ...
    yield nightmare.use(getExplanation(date)).then(result => {
      ...
      const entry = {
        date: date,
        picture: `./temp/pictures/${date}.jpg`,
        explanation: result.replace(/\n|Explanation:/g, ' ')
      };
      let jsonData = JSON.parse(
        fs.readFileSync('./temp/explanations/explanations.json', 'utf-8')
      );
      jsonData.elements.push(entry);
      fs.writeFile(
        './temp/explanations/explanations.json',
        ...
        .then(result => {
          ...
          if (result !== null) {
            return nightmare
              ...
              .wait(5000)
              .screenshot(`./temp/pictures/${date}.jpg`)
              .then(() => {
